...
jpegError (j_common_ptr cinfo)
{
  ...
  ErrorHandlerPtr myerr = (ErrorHandlerPtr) cinfo->err;
...
Frame* JpegImage::load(const std::string& filename, IResourceManager* resourceManager)
{
    Frame* res = nullptr;
    ...
    if ((infile = fopen(filename.c_str(), "rb")) != nullptr)
    {
        ...
        FrameParams params {cinfo.image_width, cinfo.image_height};
        
        res = resourceManager->createFrame(params);
        ...
        while (cinfo.output_scanline < cinfo.output_height)
        {
            ...
            memcpy(outputBuffer, buffer[0], row_stride);
            outputBuffer += res->byteSpan;
        }
...
bool JpegImage::save(const std::string& filename, const Frame& frame)
{
    bool res = false;
